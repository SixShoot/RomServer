...
bool	SqlCmdCheck( const char* sqlcmd )
{
	...
	if( str.size() > 6 )
	{
		for( int i = 0 ; i < 6 ; i++ )
		{
			...
			int  cmdlen = (int)strlen( disableStr[i] );
			while( 1 )
			{
...
bool	SqlCmdCheckW( const wchar_t* sqlcmd )
{
	...
	if( str.size() > 6 )
	{
		for( int i = 0 ; i < 6 ; i++ )
		{
			...
			int  cmdlen = (int)wcslen( disableStr[i] );
			while( 1 )
			{
...
bool Proxy_SQLBindCol(HSTMT stmt, SQLUSMALLINT colNum, int colType, SQLPOINTER targetValue, SQLINTEGER bufferLength, SQLLEN *StrLen_or_Ind)
{
	__try
	{
