...
KeyBoardCmd::KeyBoardCmd( )
{
	...
	GMS.Init( "SaveAllExit" , &KeyBoardCmd::SaveAllExit 
		, "儲存並結束程式" );
	_CmdMap[ GMS.Name ] = GMS;	
...
{
	Print( LV5 , "KeyBoardCmd::Exit" , "....."  );
    Global::Destory( EM_PlayerCenterDestroy_KeyBoardCmd_Exit );
    return true;
}
//----------------------------------------------------------------------------------------
//儲存測試
...
//存所有角色資料並退出
bool	KeyBoardCmd::SaveAllExit ( )
{
	Print( LV5 , "KeyBoardCmd::SaveAllExit" , "....."  );
	BaseItemObject::SaveAll_Block();
	Global::Destory( EM_PlayerCenterDestroy_KeyBoardCmd_SaveAllExit );
	return true;
}
//----------------------------------------------------------------------------------------
//記憶體檢查
